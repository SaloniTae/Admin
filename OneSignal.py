...
def send_push_notification(
    ...
    try:
        ...
        print(f"[ERROR] OneSignal {resp.status_code}: {resp.text}")
    ...
    def filter_transactions(
        ...
        for hour_key, txs in grouped.items():
            if txs and not upcoming.get(hour_key):
                ...
                print(f"[DEBUG][{service}] upcoming player_ids = {player_ids}")
        ...
        if new_exp and expired_list:
            ...
            player_ids = list(subs.keys())
            print(f"[DEBUG][{service}] expired player_ids = {player_ids}")
