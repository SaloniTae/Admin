...
async def show_phonepe(client, callback_query):
    # 1) Acknowledge immediately so Telegram won’t complain
    try:
        await callback_query.answer()
    ...
    # 3) Build QR + payment data off the main loop
    async def build_and_send_qr():
        # a) fetch required_amount
        user_id = callback_query.from_user.id
        ...
        # c) do all blocking work in a thread
        def sync_build():
            timings = {}
            # → generate QR
            ...
            buf = _B()
            qr.get_image(data=f"upi://pay?pa={UPI_ID}&am={amount}&pn={MERCHANT_NAME}"
                             f"&tn={order_id}&tr={order_id}&tid={order_id}",
                         image=None, optimize=20).save(buf, kind="PNG")
            buf.seek(0)
            ...
            resp_logo = _req.get(LOGO_URL)
            resp_logo.raise_for_status()
            logo      = Image.open(_B(resp_logo.content)).convert("RGBA")
            ...
            draw.rounded_rectangle((x,y,x+logo.width,y+logo.height),
            ...
            out.seek(0)
            data_uri = "data:image/png;base64," + __import__('base64').b64encode(out.getvalue()).decode()
            ...
            return img_url, timings, order_id, amount

        img_url, timings, order_id, amount = await asyncio.to_thread(sync_build)

        # e) send the QR + countdown
        ...
        # f) kickoff countdown and auto‐verify
        countdown_tasks[order_id] = asyncio.create_task(
            countdown_loop(client, msg, order_id, time.time())
        )
        asyncio.create_task(auto_verify_payment(client, msg, order_id))

    # launch in background
    asyncio.create_task(build_and_send_qr())
