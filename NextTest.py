...
async def confirm_slot_action(client, callback_query):
    ...
    2) Load referral_settings once (versioned)
    3) Conditionally load free_trial_claims (versioned)
    ...
    # ── UI fetch unchanged ───────────────────────────────────────────────────
    t0 = time.time()
    ui, is_ui_cache = await get_versioned_confirmation_ui()
    t1 = time.time()
    ui_ms = int((t1 - t0) * 1000)
    if is_ui_cache:
        logger.info(f"⏱[ConfirmSlot] UI   fetch: 0 ms (from memory cache)")
    else:
        logger.info(f"⏱[ConfirmSlot] UI   fetch: {ui_ms} ms (from Firebase)")
    ...
    # ── 2) referral_settings (versioned) ───────────────────────────────────
    t2 = time.time()
    referral_data, ref_hit = await get_versioned_referral_settings()
    t3 = time.time()
    ref_ms = 0 if ref_hit else int((t3 - t2) * 1000)
    logger.info(f"⏱[ConfirmSlot] Ref  fetch: {ref_ms} ms")
    ...
    # ── 3) free_trial_claims (versioned) ───────────────────────────────────
    needs_free = bool(referral_data.get("free_trial_enabled", False))
    if needs_free:
        t4 = time.time()
        free_trials_node, free_hit = await get_versioned_free_trial_claims()
        t5 = time.time()
        free_ms = 0 if free_hit else int((t5 - t4) * 1000)
        logger.info(f"⏱[ConfirmSlot] Free fetch: {free_ms} ms")
