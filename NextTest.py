...
async def countdown_loop(client, message, order_id: str, start_ts: float):
    ...
    while True:
        ...
        try:
            ...
            # happens if Telegram thinks the markup is identical—safe to ignore
            pass
        except Exception as e:
...
async def show_phonepe(client, callback_query):
    ...
    # 4) Send the QR **with** an initial timer button + Cancel
    initial_kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("05:00", callback_data="noop")],
        [InlineKeyboardButton("Cancel", callback_data=f"cancel_{order_id}")]
    ])
    ...
    # 5) Launch both countdown and auto-verify
