...
def get_valid_credential_for_slot(slot_id: str):
    """
    Return a tuple (cred_key, cred_data) in this priority:
      1. A global credential ("belongs_to_slot" contains "all")
      2. A slot-specific credential ("belongs_to_slot" contains slot_id)
    ...
    If any credential in scope is locked (but none valid), returns (None, "locked").
    If truly none available, returns (None, None).
    """
    locked_found = False
    found_in_all = None
    found_in_slot = None
    ...
    # 1) Fetch only the top-level keys
    try:
        resp = requests.get(REAL_DB_URL.rstrip('/') + '/.json?shallow=true', timeout=10)
        resp.raise_for_status()
        top_keys = list(resp.json().keys())
    except Exception as e:
        logging.warning(f"Failed to fetch top-level keys: {e}")
        top_keys = []

    # 2) Iterate through each key and load its node
    for key in top_keys:
        try:
            node_resp = requests.get(f"{REAL_DB_URL.rstrip('/')}/{key}.json", timeout=10)
            node_resp.raise_for_status()
            node = node_resp.json() or {}
        except Exception:
            continue

        # 3) Skip non-credential entries
        if not is_credential(node):
            continue

        # 4) Normalize belongs_to_slot into a list
        ...
        # Only consider global or matching-slot credentials
        in_all = 'all' in owns
        in_slot = slot_id in owns
        if not (in_all or in_slot):
            continue

        # 5) Check locked state
        locked_val = int(node.get('locked', 0))
        if locked_val == 1:
            locked_found = True
            continue

        # 6) Check usage and expiry
        ...
        if usage_count >= max_usage or expiry_dt <= now:
            # Already used up or expired
            continue

        # 7) Capture the first valid global or slot-specific credential
        if in_all and found_in_all is None:
            found_in_all = (key, node)
        elif in_slot and found_in_slot is None:
            found_in_slot = (key, node)

        # If we got a global one, we can stop early
        if found_in_all:
            break

    # 8) Return in priority order
    if found_in_all:
    ...
    # 9) If any were locked in scope, signal "locked"
    if locked_found:
        return None, "locked"

    # 10) Truly none available
    return None, None
...
# --------------------- Out-of-Stock Helper ---------------------
async def handle_out_of_stock(client, chat_id, user_id):
    ...
    # Send immediately (no message_queue)
...
async def choose_slot(client, callback_query):
    # 1) Acknowledge the callback_query so the client UI unblocks
    await callback_query.answer()

    user_id = callback_query.from_user.id
    slot_id = callback_query.data.replace("choose_slot_", "")

    # 2) Pull a credential (or get "locked"/None)
    cred_key, cred_data = await asyncio.to_thread(get_valid_credential_for_slot, slot_id)

    # 3a) All creds exist but are locked
    if cred_data == "locked":
        await show_locked_message(client, callback_query.message.chat.id)
        return

    # 3b) No creds left at all
    if cred_key is None and cred_data is None:
        await handle_out_of_stock(client, callback_query.message.chat.id, user_id)
        return

    # 4) Valid credential → remember slot choice and proceed
    user_slot_choice[user_id] = slot_id
...
# Load DB-based toggle values via async calls
    if await get_buy_with_points_setting():
        keyboard_rows.append([
            InlineKeyboardButton("Buy 𝗐𝗂𝗍𝗁 𝖮𝖮𝖱𝖻𝗂𝗍𝗌", callback_data="buy_with_points")
        ])

    if await get_free_trial_enabled():
        keyboard_rows.append([
            InlineKeyboardButton("𝖦𝖾𝗍 𝖥𝗋𝖾𝖾 𝖳𝗋𝖎𝖺𝗅", callback_data="free_trial")
        ])
...
# async def countdown_loop(client, message, order_id: str, start_ts: float):
    # """
    # Every second, update the inline keyboard's top button to show remaining MM:SS.
    # Stops when the order is removed (canceled/processed) or time runs out.
    # When it hits zero:
      # • delete the QR message
      # • remove the order from memory
      # • auto‐trigger /start for the user
    # """
    # chat_id = message.chat.id
    # msg_id  = message.id  # pyrogram Message.id

    # logger.info(f"[countdown_loop] started for order {order_id}, msg_id={msg_id}")

    # # Skip first tick (initial 05:00 already shown)
    # await asyncio.sleep(1)

    # while True:
        # # Stop if order no longer active
        # if order_id not in order_store:
            # logger.info(f"[countdown_loop] stopping, order {order_id} gone")
            # return

        # elapsed   = time.time() - start_ts
        # remaining = max(0, 300 - int(elapsed))
        # if remaining <= 0:
            # logger.info(f"[countdown_loop] time up for order {order_id}")

            # # ── 1) delete the QR message ───────────────────────────────
            # try:
            ...
            # # ── 3) auto‐trigger /start for this user ───────────────────
            ...
            # class _FakeMsg:
                # def __init__(self, chat_id, user):
                    ...
                    # self.from_user = user
                    # self.text = "/start"
            # fake_message = _FakeMsg(chat_id, message.chat)
            # await start_command(client, fake_message)
        ...
        # mm = remaining // 60
        # ss = remaining % 60
        # # split each digit and space it out, with spaced colon
        # timer_text = f"{mm//10} {mm%10} : {ss//10} {ss%10}"
        # logger.info(f"[countdown_loop] order {order_id} → {timer_text}")
        ...
        # except MessageNotModified:
            ...
            # pass
        # except Exception as e:
            # logger.error(f"[countdown_loop] edit failed: {e!r}")
            # return
...
CAPTION2 = "𝗙𝗼𝗿 𝗣𝗮𝘆𝗺𝗲𝗻𝘁 𝗚𝗮𝘁𝗲𝘄𝗮𝘆 𝖼𝗈𝗇𝗍𝖺𝖼𝗍 𝗎𝗌 @oor_agent"
